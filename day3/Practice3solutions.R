...
par(xpd=TRUE)
barplot(ucb_table, beside = TRUE,xlab = "Gender",col = c("red","orange"))
legend(x=4,y=2100, rownames(ucb_table),fill=c("red","orange"),title = "Acceptance UCB by gender")

par(xpd=TRUE)
barplot(ucb_table,col = c("red","orange"))
legend(x=1.5,y=3500, rownames(ucb_table),fill=c("red","orange"),title = "Acceptance UCB by gender")
...
par(xpd=TRUE)
barplot(t(ucb_table),xlab = "Admit Status",col = c("red","orange"))
legend(x=.25,y=4000, colnames(ucb_table),fill=c("red","orange"),title = "Gender")
